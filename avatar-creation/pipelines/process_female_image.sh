#!/bin/bash
# COMPLETE FEMALE PIPELINE: woman.jpg → 3D Mesh → A-pose → T-shirts → Heatmaps → Texture → GLB
# Adapted from run_full_pipeline.sh for female avatar
...
echo "============================================================"
echo "🚀 COMPLETE FEMALE AVATAR PIPELINE"
echo "============================================================"
...
if [ -n "${FACE_IMG_PATH}" ] && [ -f "extract_skin_tone.py" ]; then
    ...
    if [ -f "${OUTPUT_DIR}/body_woman_apose_textured.obj" ]; then
...
for size in XS S M L XL; do
    ...
    python generate_playboy_tshirt_PERFECT.py \
        --smpl-params "${FINAL_PARAMS}" \
        --gender "${GENDER}" \
        --size "${size}" \
        --output "${TSHIRT_DIR}" \
        --arm-angle 45 \
        --fit-adjustment 0.0 2>&1 | grep -E "(✅ Saved|Garment:|Body:)" || echo "   Processing ${size}..."
done
deactivate
