...
HANDLER_BUILD = (
    ...
    "logos on PT_FABRIC_* materials kept rendering invisible. v38 stops "
    "fighting MTLLoader's preload pipeline and builds materials manually. "
    "We still call MTLLoader.parse(mtlText, '') to get materialsInfo (the "
    "raw MTL data per material name with kd/ka/map_kd fields) because "
    "that part is reliable. We then construct THREE.MeshStandardMaterial "
    "ourselves for each material name, load textures via THREE.TextureLoader "
    "with explicit colorSpace=SRGBColorSpace, and attach them to mat.map. "
    "Finally we override materials.materials, materials.create, and "
    "materials.preload so OBJLoader's lookup returns our hand-built "
    "materials and never invokes MTLLoader's flow. This is bulletproof. "
    "If textures still do not show after v38, the issue is geometric "
    "(UVs not surviving the OBJ pipeline, prints too small to see at this "
    "render distance, or face references missing vt indices) rather than "
    "the loader. No handler-side code changes in v38; banner update only. "
    "PREVIOUS v37 BANNER FOLLOWS: "
